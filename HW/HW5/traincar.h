...
public:
  ...
  static TrainCar* MakeEngine()               { return new TrainCar('e',150);    }//e for engine, weight of 150 tons
  static TrainCar* MakeFreightCar(int weight) { return new TrainCar('f',weight); }//f for freight, weight of GIVEN WEIGHT
  static TrainCar* MakePassengerCar()         { return new TrainCar('p',50);     }//p for passenger, weight of 50 tons
  static TrainCar* MakeDiningCar()            { return new TrainCar('d',50);     }//d for dining, weight of 50 tons
  static TrainCar* MakeSleepingCar()          { return new TrainCar('s',50);     }//s for sleeping, weight of 50

  // ACCESSORS
  int getID() const           { return id; }//get the id
  int getWeight() const       { return weight; }//get the weight
  
  bool isEngine() const       { return (type == 'e'); }//check if engine
  bool isFreightCar() const   { return (type == 'f'); }//check if freight
  bool isPassengerCar() const { return (type == 'p'); }//check if passenger
  bool isDiningCar() const    { return (type == 'd'); }//check if dining
  bool isSleepingCar() const  { return (type == 's'); }//check if sleeping
