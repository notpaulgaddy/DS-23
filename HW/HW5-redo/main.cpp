...
void PrintTrain(TrainCar *train) {
  ...
  TotalWeightAndCountCars(train, total_weight, num_engines, num_freight_cars,
                          num_passenger_cars, num_dining_cars,
                          num_sleeping_cars);
...
void SimpleTrainTest() {
  ...
  assert(simple->next->next->next->next->getID() ==
         simple->next->next->next->getID() + 1);
  assert(simple->next->next->next->next->next->getID() ==
         simple->next->next->next->next->getID() + 1);
...
void ShipFreightHelper(MTRand_int32 &mtrand, int num_engines, int num_cars,
  ...
  for (int i = 0; i < num_cars; i++) {
    // the weight for each car is randomly generated in the range of 30->100tons
    int weight = 30 + (mtrand() % 15) * 5;
    TrainCar *temp = TrainCar::MakeFreightCar(weight);
  ...
  std::vector<TrainCar *> trains =
      ShipFreight(all_engines, all_freight, min_speed, max_cars_per_train);
  ...
  for (unsigned int i = 0; i < trains.size(); i++) {
    ...
    int total_weight, num_engines, num_freight_cars, num_passenger_cars,
        num_dining_cars, num_sleeping_cars;
    TotalWeightAndCountCars(trains[i], total_weight, num_engines,
...
void SeparateStatistics(const std::vector<int> &original,
                        ...
                        const std::vector<int> &right, int &num_unlinks,
                        int &num_links, int &num_shifts) {
  num_unlinks = 0;
  num_links = 0;
   num_shifts = 0;
  ...
  for (int i = 0; i < int(original.size()); i++) {
    ...
    if (i == int(original.size()) - 1 && after != -1) {
      num_links++;
    }
    // middle links
    if (i > 0) {
...
void SeparateTestHelper(TrainCar *&train1, const std::string &which_test) {
  ...
  if (speed_left < 0.99 * speed_original) {
    ...
    std::cout << "left train is " << std::setprecision(1) << std::fixed
              ...
              << std::setprecision(1) << std::fixed << -right_percent
              << "% faster than the original." << std::endl;
    ...
    std::cout << "right train is " << std::setprecision(1) << std::fixed
              ...
              << std::setprecision(1) << std::fixed << -left_percent
              << "% faster than the original." << std::endl;
  } else {
