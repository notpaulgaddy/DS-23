...
// int count_paths_block(const std::vector<std::vector<GRID_STATUS> >& blocked_grid,int x, int y) {
//   if (x < 0 || y < 0) {
//         return 0;
//     }
...
//         return 1;
//     }
//     int paths_from_above = count_paths_block(blocked_grid, x-1, y);
//     int paths_from_left = count_paths_block(blocked_grid, x, y-1);
...
int main(int argc, char* argv[]) {
  ...
  // int num_paths_block = count_paths_block(blocked_grid, 2, 2);
  // std::cout << "Number of paths: " << num_paths_block << std::endl;

  int totalPaths = count_paths(3,3);
  std::cout << "Number of paths: " << totalPaths << std::endl;
