...
bool find_roots(int a, int b, int c, float &root_pos, float &root_neg)
{
  ...
  root_pos = (-b + sqrt_radical) / (2 * a); // added the type
  root_neg = (-b - sqrt_radical) / (2 * a); // added the type
  return true;
}
int main()
{
  ...
  while (true)
  {
    ...
    // Check our work...
    if (check_root(my_a, my_b, my_c, root_1) && check_root(my_a, my_b, my_c, root_2)) // fixed the () issue
    {
